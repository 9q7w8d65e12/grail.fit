const keywords = ["搭配邏輯", "虛擬試穿", "風格配置", "色彩美學", "剪裁分析"];
const feedbacks = ["非常精準", "科技感十足", "令人驚艷", "省下超多時間", "非常有參考價值"];
const userNames = ["穿搭愛好者", "潮流玩家", "科技宅", "時尚設計師", "Grail用戶"];

function generateAIFeedback() {
    const kw = keywords[Math.floor(Math.random() * keywords.length)];
    const fb = feedbacks[Math.floor(Math.random() * feedbacks.length)];
    const name = userNames[Math.floor(Math.random() * userNames.length)];
    return `「${kw}${fb}，推薦大家試試看！」 — ${name} ${Math.floor(Math.random()*900+100)}`;
}
